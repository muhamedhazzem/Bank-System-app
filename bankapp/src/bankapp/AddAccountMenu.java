        ...
        if (warnings.length() > 0) {
            ...
            if (typeField.getSelectedItem().toString() == "Checking") {
                if (amount >= 100) {
                    ...
                    warnings.append("Initial deposit must be at least EGP100 for Checking accounts.");
                }
            } else if (typeField.getSelectedItem().toString() == "Savings") {
                if (amount >= 50) {
                    account = new Savings(amount);
                } else {
                    warnings.append("Initial deposit must be at least EGP50 for Savings accounts.");
                }
            }
            if (account != null) {
